...
def _get_from_saved_model(graph_func, input, print_result=False):
    output_data = graph_func(input)[0].numpy()
    if print_result:
        print(output_data)
    return output_data
...
def tflite_model_predict(request, file_path, mpi_size=1, mpi_rank=0, comm=None):
    shard = [mpi_size, mpi_rank]
    ds_val = get_dataset(prune_conf_from_obj(request), is_training=False, shard=shard)
...
def tftrt_model_predict(request, file_path):
    ...
    score = 0
    for x_test, y_test in val_dataset:
        input = tf.convert_to_tensor(x_test)
        _out = _get_from_saved_model(graph_func, input)
